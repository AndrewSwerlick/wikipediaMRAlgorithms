...
public class TopLevelReferenceDomainCategorizer {
	public static class MapClass extends Mapper<LongWritable, Text, Text, LongWritable>{
		...
		public void map(LongWritable key, Text value, Context output) throws InterruptedException, IOException{
			...
			if(urlMatcher.find())
			{
				...
				if(domainParts.length > 1 && domainParts[domainParts.length-1] != "uk")
				{
	...
	public static void runJob(Path input, Path output) throws IOException, InterruptedException, ClassNotFoundException{
        ...
        job2.setJarByClass(TopLevelReferenceDomainCategorizer.class);
        job2.setMapperClass(TopLevelReferenceDomainCategorizer.MapClass.class);
